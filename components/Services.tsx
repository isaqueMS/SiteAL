...
const Services: React.FC<ServicesProps> = ({ onOpenBooking }) => {
  ...
  const ServiceCard = ({ title, items, icon, colorClass }: any) => (
    <div className="card-modern p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem]">
      <div className={`w-14 h-14 md:w-16 md:h-16 ${colorClass} rounded-2xl flex items-center justify-center mb-6 md:mb-8 shadow-lg shadow-pink-500/10`}>
        {icon}
      </div>
      <h3 className="text-2xl md:text-3xl font-serif text-white mb-6 md:mb-8">{title}</h3>
  ...
  return (
    <div className="container mx-auto px-6 md:px-12">
      ...
      <div className="mt-12 md:mt-20 p-6 md:p-12 bg-gradient-to-r from-pink-500/10 to-transparent rounded-[2rem] md:rounded-[2.5rem] border border-pink-500/20 flex flex-col md:flex-row items-center justify-between gap-8">
        <div>
          <h4 className="text-xl md:text-2xl font-serif text-white mb-2 text-center md:text-left">Especialistas em Alongamentos & Alisamentos</h4>
          <p className="text-stone-400 font-light max-w-xl text-center md:text-left text-sm md:text-base">
            Do Mega Hair aos Alisamentos de alta performance, garantimos resultados naturais que preservam a saúde do seu fio.
          </p>
        </div>
        <button 
          onClick={onOpenBooking}
